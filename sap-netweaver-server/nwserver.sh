...
nw_prereqs  $SUBEMAIL $SUBID $SUBURL $ASCSSID $ASCSINSTANCE $ERSINSTANCE $SAPBITSMOUNT $SAPMNTMOUNT $USRSAPSIDMOUNT $USRSAPASCSMOUNT $USRSAPERSMOUNT \
    $SAPINSTGID $SAPSYSGID $VMIPADDR $VMNAME $NFSILBIP $ASCSILBIP $DBIP $DBHOST
download_sapbits $URI "/sapbits"
write_nw_ini_file $VMNAME $MASTERPASSWD $SAPADMUID $SAPSYSGID $SIDADMUID $DBHOST $DBSID $DBINSTANCE $ASCSSID $ASCSILBIP $NWINSTANCE
